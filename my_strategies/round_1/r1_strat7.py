...
# ============================================================
# r1_strat7.py  —  Bug-fixed version of strat6 pepper logic.
#
# ROOT CAUSE OF STRAT6 REGRESSION (diagnosed by backtest):
#   strat6 quoted only PEPPER_LOT=10 lots per side, while
#   strat4/5 quoted a full 80 lots each side.
#   Spread capture = lot_size × spread_per_tick:
#     strat4: 80 × 13 ≈ 1,040 / tick
#     strat6: 10 × 13 ≈   130 / tick  (−87.5%)
#   Additionally, the TREND_TARGET size-skew further cut sell
#   lots to 5 when pos < 20, making inventory accumulate.
#
# THE FIX — keep all the good ideas, fix the execution:
#
#   ✓ KEEP: zigzag regime detection (lag-1 autocorr = -0.49)
#   ✓ KEEP: traderData to carry last mid-price across ticks
#   ✓ KEEP: one-sided market guard
#
#   ✗ REMOVE: size-skewing (LOT=10 and halving sell_lots)
#
#   ★ NEW: price-skewing instead of size-skewing
#     Always quote FULL capacity on both sides (like strat4/5).
#     Apply trend + zigzag logic by SHIFTING BID/ASK PRICES:
#
#     Trend layer  → shift BOTH bid and ask up by TREND_SHIFT=1
#                    (makes us cheaper to buy from us, harder
#                     to sell to us → net long drift over time)
#
#     Zigzag layer → after UP tick: tighten ask (ask−1 extra),
#                    relax bid (bid−1); expect a down-tick
#                  → after DOWN tick: tighten bid (bid+1 extra),
#                    relax ask (ask+1); expect an up-tick
#                  → flat: standard symmetric pennying
#
#   Result: full 80-lot volume on both sides at all times,
#   while directional signal is expressed purely via price.
# ============================================================
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            # ═══════════════════════════════════════════════
                ...
                # Regime-specific readable version:
